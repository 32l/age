...
def visualize_model(model, dset = None, num_sample = 100, output_dir = None):
    '''
    ...
    for idx in xrange(num_sample):
        ...
        feat_res = model.G_net(feat_int)
        feat_diff = feat_real - feat_in
        
        print(feat_in.size())
        print(feat_real.size())
        print(feat_res.size())
        print(feat_diff.size())
        ...
        img = mpimg.imread(dset.video_lst[idx]['frames'][0])
        ax.imshow(img)
        ...
        ax.set_ylim([-10,10])
        ax.plot(feat_in.data.numpy().flatten())
        ax.set_xlabel('feat')
        
        # feat_diff
        ax = fig.add_subplot(4,1,3)
        ax.set_ylim([-10, 10])
        ax.plot(feat_diff.data.numpy().flatten())
        ax.set_xlabel('feat_res_real')
        
        # feat_res
        ax = fig.add_subplot(4,1,4)
        ax.set_ylim([-10, 10])
        ax.plot(feat_res.data.numpy().flatten())
        ax.set_xlabel('feat_res_gen')
