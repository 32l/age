...
def age_dis_layer():
    ...
    layer.weight.data = torch.Tensor(weight).float().view(70,70).transpose(0,1)
    layer.bias.data = torch.Tensor(bias).float()
...
class KLLoss(nn.Module):
    ...
    def __init__(self):
        ...
        self.age_dis_trans = age_dis_layer()
    ...
    def forward(self, fc, seq_len):
        ...
        log_prob = F.log_softmax(self.age_dis_trans(F.sigmoid(fc)))
        ...
        for i in range(bsz):
            l = seq_len.data[i]-1
            loss = loss + F.kl_div(log_prob[i,0:l], prob[i,1:(l+1)], False)
        loss = loss / bsz
        return loss